...
def _append_candidate(candidates, value):
    ...
    if os.path.isdir(value):
        candidates.extend([os.path.join(value, "1cv8"), os.path.join(value, "1cv8.exe")])
    else:
        candidates.append(value)


def resolve_v8path(v8path):
    """Resolve path to a 1C binary that supports DESIGNER mode."""
    candidates = []
    if v8path:
        _append_candidate(candidates, v8path)
    else:
        _append_candidate(candidates, os.environ.get("V8_PATH"))
        _append_candidate(candidates, os.environ.get("V8PATH"))
        if os.name == "nt":
            candidates.extend(glob.glob(r"C:\Program Files\1cv8\*\bin\1cv8.exe"))
        else:
            _append_candidate(candidates, shutil.which("1cv8"))
            candidates.append("/opt/1cv8/current/1cv8")
            for pattern in (
    ...
    print("Error: 1cv8 binary not found. Specify -V8Path or set V8_PATH.", file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    try:
        ...
        # --- Execute ---
        print(f"Running: {v8path} {' '.join(arguments)}")
