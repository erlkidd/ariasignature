...
- публиковать произведенные данные через REST API (по умолчанию **доступный по сети** на всех интерфейсах узла, с опцией только localhost и опциональным токеном для удалённых клиентов);
...
- Настройка порта API, привязки сокета (сеть / localhost), опционального токена для удалённых запросов; расписание диагностики.
