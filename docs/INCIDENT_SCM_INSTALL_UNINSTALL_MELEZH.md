...
6. Проверка `InstallHealthStatus <> 'install-health:ok'` прерывала установку при любом `degraded-*`, хотя ниже были сообщения про «ограниченную готовность».
7. Удаление `{app}` полагалось только на `[UninstallDelete]` без `DelTree` после остановки процессов в `usPostUninstall`.
...
- Post-install: `RaiseException` только при `install-health:fail-hard*`; Melezh UI probe → `degraded-melezh-ui`.
