...
- Релиз **1.0.0**: удалённый HTTP API по IP/VPN (`Api:Bind`, опциональный токен в настройках), правило брандмауэра в установщике, интеграционные тесты и `scripts/api-http-smoke.ps1`.
- UI: формулировки настроек — «токен для удалённого API»; описание Bearer в Swagger.

### Documentation sync

- Документы и версии сборок синхронизированы по **1.0.0**.

## Release 0.9.9

### Highlights

- Релиз **0.9.9**: мягкий старт UI без ложных ошибок API; параллельный старт службы; автозапуск только через галочку (HKCU + ярлык); установщик без user Startup / schtasks для UI.

### Documentation sync

- Документы синхронизированы по версии `0.9.9`.
