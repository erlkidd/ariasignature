...
- `GET /settings`
  ...
  - Поля:
    ...
    - `outboundBearerToken`: `null`, если не задан, иначе маска `"***"` (секрет не раскрывается);
    - `outboundCustomHeaderName`, `outboundCustomHeaderValue` (значение секретное — маска `"***"` при наличии).
...
- `PUT /settings`
  ...
  - Тело (все поля необязательны; отсутствующие не меняются):
    ...
    - `outboundBearerToken`: новое значение; пустая строка — удалить; если значение равно маске `"***"` из `GET`, токен **не меняется** (оставьте поле вне тела запроса, если не хотите менять секреты — см. клиент UI);
    - `outboundCustomHeaderName`, `outboundCustomHeaderValue` (аналогично секрету значения заголовка и маске `"***"`).

Локальный входящий API **не** проверяет Bearer и пользовательские заголовки из этого блока — они используются **только** при исходящих запросах на коллектор.
...
Заголовки запроса к коллектору: при наличии настроек добавляются `Authorization: Bearer …` и при необходимости пользовательский заголовок (`outboundCustomHeaderName` / `outboundCustomHeaderValue`).
