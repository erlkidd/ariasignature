...
- `GET /settings`
  ...
  - Поле `note`: напоминание, что смена порта API действует после перезапуска службы `AriaSignatureService`; cron обновления дисков применяется сразу при сохранении через `PUT /settings`.
