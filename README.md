...
- **Опциональная защита:** общий секрет в настройках — для запросов **не с localhost** требуется `Authorization: Bearer` или `X-Aria-Api-Key`; локальная панель на том же ПК заголовки не задаёт.
...
**Кто может дергать API?** Любой узел, который может установить TCP-сессию на порт агента. Ограничивайте доступ VPN/брандмауэром; при необходимости задайте общий секрет в настройках службы. Не выставляйте порт в открытый интернет без обратного прокси и политики доступа.
